...
int contaNodiConDueFigli(nodo* root){
    if(root == nullptr) return 0;
    int cont = 0;
    if(root->sinistro != nullptr && root->destro != nullptr) cont++;
    cont += contaNodiConDueFigli(root->sinistro);
    cont += contaNodiConDueFigli(root->destro);
...
int main(){
    ...
    int cont = contaNodiConDueFigli(root);
    cout << "Numero di nodi con due figli: " << cont << endl;
